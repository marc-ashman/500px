...
/*
 * Revised 12/29/2010 by Marc Ashman
 * Changed to be a image-only 2d scroller, with added pinch-zoom controls
 */
...
public class FullImageViewContainer extends FrameLayout {
    public static final int ZOOM_TYPE_ABSOLUTE = 1;
    public static final int ZOOM_TYPE_ADDITIVE = 2;
    ...
    /**
     * True if the user is currently dragging this View around. This is
    ...
    private ScaleGestureDetector gestureDetector = new ScaleGestureDetector(getContext(), new ScaleGestureDetector.OnScaleGestureListener()
    {
        @Override
        public boolean onScale(ScaleGestureDetector detector)
        {
            ...
            float spanDelta = detector.getScaleFactor();

            int scrollX = (int) (((getScrollX() + x) * spanDelta) - x);
            int scrollY = (int) (((getScrollY() + y) * spanDelta) - y);
    ...
    public FullImageViewContainer(Context context)
    {
        this(context, null);
    }

    public FullImageViewContainer(Context context, AttributeSet attrs)
    {
        this(context, attrs, android.R.attr.scrollViewStyle);
    }

    public FullImageViewContainer(Context context, AttributeSet attrs, int defStyle)
    {
    ...
    public void zoomBy(int zoomType, float scale)
    {
        final int originalWidth = image.getDrawable().getIntrinsicWidth();
        final int originalHeight = image.getDrawable().getIntrinsicHeight();
        int width;
        int height;
        float zoom;
        switch(zoomType)
        {
            case ZOOM_TYPE_ADDITIVE:
                        /*
                         * This function has the potential of being called many times, so the
                         * conversion from float -> int creates distorted images with different
                         * aspect ratios from the original.
                         *
                         * To correct this, we instead calculate the width, then take the
                         * delta between it and the original picture's width, then calculate
                         * the height with the delta.
                         */
                width = (int) (contentWidth * scale);

                zoom = (float) width / (float) originalWidth;
                if(zoom > maxZoom)
                {
                    zoom = maxZoom;
            ...
            case ZOOM_TYPE_ABSOLUTE:
                ...
                break;
            default:
                throw new IllegalArgumentException("invalid zoomType argument");
        }
        zoomScale = zoom;
    ...
    public void setImage(Bitmap bitmap, float minZoom, float maxZoom)
    {
        ImageView image = new ImageView(getContext());
        image.setImageBitmap(bitmap);
        setImage(image, minZoom, maxZoom);
    }

    public void setImage(int resId, float minZoom, float maxZoom)
    {
        ImageView image = new ImageView(getContext());
    ...
    public void setImage(Drawable drawable, float minZoom, float maxZoom)
    {
        ImageView image = new ImageView(getContext());
    ...
    public void setImage(ImageView image, float minZoom, float maxZoom)
    {
        this.image = image;
        setZoomLimits(minZoom, maxZoom);
        frame.removeAllViews();
        frame.addView(image);
        zoomBy(ZOOM_TYPE_ABSOLUTE, 1);
        centerTo(image.getDrawable().getIntrinsicWidth() / 2,
                image.getDrawable().getIntrinsicHeight() / 2);
    }

    /**
     * Calling this can cause epic failures to occur in this View, <b><u>DO NOT USE IT</u></b>.
     * Instead, use {@link #setImage(android.widget.ImageView)} to set the contents of the View
     */
    @Override
    @Deprecated
    public void addView(View child)
    {
        if(getChildCount() > 0)
        {
            throw new IllegalStateException(this.getClass().getName() + " can only hold one direct child");
        }
        super.addView(child);
    }
    ...
    /**
     * @return Returns true this View can be scrolled
     */
    ...
    private void scrollToChild(View child)
    {
        child.getDrawingRect(mTempRect);
                /* Offset from child's local coordinates to View coordinates */
    ...
    /**
     * set zoom limits to the image, and re-zoom within appropriate limits
     * @param maxZoom
     * @param minZoom
     */
    public void setZoomLimits(float minZoom, float maxZoom)
    {
        ...
        zoomBy(ZOOM_TYPE_ABSOLUTE, zoomScale);
    }
